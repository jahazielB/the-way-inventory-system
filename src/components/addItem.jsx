...
export const AddItem = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.item_name.trim() || !formData.unit.trim()) {
      setSnackbar({
        open: true,
        type: "error",
        message: "Item and Unit Required!",
      });
      return;
    }
    try {
      ...
      const { data, error } = await supabase.rpc("manage_item", {
        ...
        p_opening_stock: formData.opening_stock
          ? parseFloat(formData.opening_stock)
          : 0,
        p_reorder_point: formData.reorder_point
          ? parseFloat(formData.reorder_point)
          : 0,
        p_reorder_quantity: formData.reorder_quantity
          ? parseFloat(formData.reorder_quantity)
          : 0,
  ...
  return (
    <div
      className="w-[clamp(100px,80vw,600px)] items-center md:w-[600px] lg:w-[clamp(200px,50vw,533px)] h-[400px] xl:h-[467px] mx-auto my-auto  
    ...
    >
        ...
        <Select
          ...
          renderValue={(selected) => {
            if (selected.length === 0) {
              return (
                <span className="text-gray-400 ml-39">Choose Customers</span>
              );
            }
      ...
      <div className="createAccountTextArea flex items-center justify-center">
        <select
          ...
          name="location_id"
          onChange={(e) => handleChange(e)}
          className="border-none focus:outline-none rounded w-36 py-2 "
        >
          <option value="" disabled hidden>
            Choose Location
          </option>
          {datas &&
            (datas?.locations || []).map((d, index) => (
      ...
      <button
        ...
        disabled={loading}
        className="w-[clamp(100px,70vw,350px)] md:h-[50px] md:w-full h-[40px] lg:h-[55px] xl:h-[65px] bg-blue-600 rounded-2xl text-white hover:bg-[rgba(37,99,235,.8)]
                         active:bg-[rgba(12,51,137,0.8)] active:scale-99"
      >
