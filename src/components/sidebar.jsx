...
export const Sidebar = ({ data }) => {
  ...
  const handleLogout = async () => {
    ...
    if (user.id) {
      const { error } = await supabase
        .from("users")
        .update({ is_logged_in: false, last_active: new Date() })
        .eq("id", user.id);
  ...
  return (
    <div className="flex z-[1301] max-lg:fixed max-lg:w-[200px] lg:w-[256px]  ">
      {/* Sidebar */}
      <div
        className={`h-full z-[1300]  w-54 md:w-[256px]  transform transition-transform duration-300 md:min-h-screen bg-white text-[rgba(0,0,255,90)] max-lg:fixed flex flex-col
      ...
      >
        ...
        <div>
          ...
          <div className="relative p-4 pb-20  ">
            <img
              src="/word.png"
              className="absolute inset-0 z-0 opacity-80 mx-auto pointer-events-none"
              alt=""
            />
          </div>
          ...
          <nav className="mt-8 ml-2 md:ml-0">
            <ul className="space-y-1.5 ">
              <li
                className={`dashboard-menu  text-[15px] md:text-[20px]  ${location.pathname === "/dashboard" && "bg-gray-300 font-black"}`}
                onClick={() => temporaryHandleClick("dashboard")}
              >
                <svg
              ...
              <li
                className={`dashboard-menu submenu ${location.pathname === "/projects" || location.pathname === `/projects/${encodeURIComponent(projectMatch?.params.projectName)}` ? "bg-gray-300 font-black" : ""}`}
                onClick={() => temporaryHandleClick("projects")}
              >
                <svg
                  width="18"
                  height="18"
              ...
              <div className="pl-19 my-2">
                <span className=" text-[12px] ">
                  {projectMatch?.params?.projectName}
                </span>
              </div>

              <li
                className={`dashboard-menu submenu ${location.pathname === "/inventory" || location.pathname === `/inventory/item_summary/${encodeURIComponent(inventoryMatch?.params.inventoryName)}` || location.pathname === "/inventory/item_summary" ? "bg-gray-300 font-black" : ""}`}
                onClick={() => temporaryHandleClick("inventory")}
              >
                <svg
                  width="18"
                  height="18"
              ...
              <div className="pl-19 my-2">
                <span className=" text-[12px] ">
                  {location.pathname === "/inventory/item_summary"
                    ? "General"
                    : inventoryMatch?.params?.inventoryName}
                </span>
              </div>
              <li
                className={`dashboard-menu submenu ${location.pathname === "/accounts" || location.pathname === "/accounts/create" ? "bg-gray-300 font-black" : ""}`}
                onClick={() => temporaryHandleClick("accounts")}
              >
                <svg
                  width="18"
                  height="18"
        ...
        <div className="flex-col p-4  ml-5 my-7 text-[11px] md:text-[13px] text-blue">
          <NotificationBell location={location.pathname} />
          <div className="flex gap-2 mb-2">
            ...
            >
              <path
                d="M14.6667 6.41667C14.6667 8.44171 13.025 10.0833 11 10.0833C8.97496 10.0833 7.33333 8.44171 7.33333 6.41667C7.33333 4.39162 8.97496 2.75 11 2.75C13.025 2.75 14.6667 4.39162 14.6667 6.41667Z"
                stroke="white"
                stroke-width="1.2"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M11 12.8333C7.45617 12.8333 4.58333 15.7062 4.58333 19.25H17.4167C17.4167 15.7062 14.5438 12.8333 11 12.8333Z"
                stroke="white"
                stroke-width="1.2"
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <div
            className="flex gap-3 cursor-pointer active:bg-amber-600 hover:bg-[rgba(107,107,182,0.9)] rounded-4xl"
          ...
          >
            ...
            >
              <path
                d="M0.962406 8.01765L0.37594 7.54773L0 8.01765L0.37594 8.48758L0.962406 8.01765ZM7.72932 8.76953C7.92873 8.76953 8.11998 8.69032 8.26098 8.54931C8.40199 8.40831 8.4812 8.21706 8.4812 8.01765C8.4812 7.81824 8.40199 7.627 8.26098 7.48599C8.11998 7.34499 7.92873 7.26577 7.72932 7.26577V8.76953ZM3.38346 3.78833L0.37594 7.54773L1.54887 8.48758L4.55639 4.72818L3.38346 3.78833ZM0.37594 8.48758L3.38346 12.247L4.55639 11.3071L1.54887 7.54773L0.37594 8.48758ZM0.962406 8.76953H7.72932V7.26577H0.962406V8.76953Z"
                fill="#0118D8"
              />
              <path
                d="M6.97744 5.10931V4.55066C6.97744 3.33337 6.97744 2.7251 7.33383 2.3048C7.69023 1.8845 8.29023 1.78375 9.49023 1.58299L10.7474 1.37397C13.1857 0.967956 14.4045 0.764949 15.2023 1.44014C16 2.11608 16 3.35217 16 5.8236V10.2108C16 12.683 16 13.9191 15.203 14.5943C14.4045 15.271 13.1857 15.068 10.7474 14.6612L9.49023 14.4514C8.29023 14.2514 7.69023 14.1514 7.33383 13.7311C6.97744 13.3101 6.97744 12.701 6.97744 11.4837V11.074"
                stroke="#0118D8"
                stroke-width="2"
              />
            </svg>
      ...
      {!isOpen && (
        <svg
          className="absolute  lg:hidden cursor-pointer active:scale-95"
          width="60px"
          height="60px"
          viewBox="0 0 24 24"
          fill="none"
